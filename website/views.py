...
def products_view(request):
    ...
    if request.method == "POST":
        ...
        products = (
            ...
            .filter(name__contains=search_word, **filter_kargs)
            .distinct()
            .order_by("id")
        )
...
def product_details(request, slug, id):
    ...
    sproducts = (
        Product.objects.language(request.LANGUAGE_CODE)
        .order_by("id")
    ...
    return render(
        ...
        {"product": product, "sproducts": sproducts},
    )
...
def blogs_view(request):
    ...
    if request.method == "POST":
        ...
        blogs = (
            Blog.objects.language(lang)
            .filter(translations__title__contains=search_word, **filter_kargs)
            .distinct()
            .order_by("id")
        )
...
def blog_detail(request, slug, id):
    ...
    sposts = (
        Blog.objects.language(request.LANGUAGE_CODE).all().order_by("created_at")[:5]
    )
...
def subscribe(request):
    if request.method == "POST":
        ...
        if shalf[0] == "" or len(shalf) != 2 or shalf[1].count(".") != 1:
            return JsonResponse({"message": _("invalid email"), "status": "danger"})
        ...
        return JsonResponse(
            {
                "message": _("Thanks for subscribing to our Newsletter"),
                "status": "success",
            },
            status=201,
        )
